...
struct
  ...
  (* Upward phase for w where its parent is a Three node whose (k,v) is x.
   ...
   *
   * From our handout, for the first case where w's parent is a Three-tree,
      ...
      match D.compare k k1  with
       ...
       | Less -> 
           (match insert_downward left k v with 
             ...
             | Done a -> Done (Two(a, x, right))   
           )
       | Greater -> 
           (match insert_downward right k v with 
             | Up(m,w,r) -> 
               insert_upward_two w m r (k1,v1) left      
             | Done a -> 
               Done (Two(left, x, a))
           )
  ...
  let rec height (d: dict) : int option = 
    ...
    | Three(l, _, m, _, r) -> 
      ...
      (match height_l with
        | None -> None
        | Some h -> if height_l = (height m) && height_l = (height r) 
...
(*    Printf.printf "%s\n" (string_of_dict d1);
    Printf.printf "%s\n" (string_of_tree d1);*)
    List.fold_right (fun x acc -> assert (lookup d1 (fst x) = Some (snd x))) 
                    members ();
  ...
  let test_inserts () =
    (*print_string "*** Begin Tests of inserts ***\n";*)
    let int_to_test = [27; 41; 63; 80; 61; 50; 19; 55; 11; 42; 53; 81; 54] in 
    let pairs_1_to_100 = generate_pair_list 100 in
    let pairs_filtered = List.filter ( fun (k,v) ->  
      (List.exists (fun x -> x = int_of_string(string_of_key k)) int_to_test)
     ) pairs_1_to_100 in  
    let pairs_filtered_ordered=List.map 
      (fun x -> 
        List.find (fun (k,v) -> int_of_string(string_of_key k) = x) 
                  pairs_filtered
      ) int_to_test in
    ...
    (* Both trees should been the same *)
