...
struct
  ...
  and insert_downward_two ((k,v): pair) ((k1,v1): pair) 
      ...
      match D.compare k k1  with
       ...
       | Less -> (*print_string "enter eq | less";*)
  ...
  and insert_downward_three ((k,v): pair) ((k1,v1): pair) ((k2,v2): pair) 
      ...
      match D.compare k k1 with
        | Eq -> Done ( Three(left, (k1, v), middle, (k2, v2), right) ) 
    | Less -> (*print_string "enter eq less \n";*)
           (match insert_downward left k v with 
             | Up(m,w,r) ->
        ...
        | Greater -> (*print_string "enter pre gt \n";      *) 
          (match D.compare k k2 with
             | Eq -> Done ( Three(left, (k1, v1), middle, (k2, v), right) ) 
         | Less -> (*print_string "enter gt less \n"; *)  
   ...
   * ANSWER: 
   *    Caclulate the height of the tree recursively.
   *    If subtrees are of a different height, height returns None
   *    For each tree level, add 1 to the height
   *    If subtrees are of an equal height recursively (for each 2-node and 3-node), then the tree is balanced
   *)
  ...
  (* adds a list of (key,value) pairs in left-to-right order *)
  let insert_list (d: dict) (lst: (key * value) list) : dict = 
    List.fold_left (fun r (k,v) -> (*(Printf.printf "insert_list: %s\n" (string_of_tree r));*) 
      insert r k v) d lst
...
(*    Printf.printf "%s\n" (string_of_tree d1);
    Printf.printf "%s\n" (string_of_dict d1);*)
    List.iter (fun (k,v) -> 
(*      (Printf.printf "looking up key %s\n" (string_of_key k));
      (match lookup r2 k with 
      | None -> Printf.printf "None\n"
      | Some vv -> Printf.printf "Found value %s\n" (string_of_value vv));*)
